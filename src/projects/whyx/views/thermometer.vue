<template>
  <div v-if="!showChildrenPage">
    ...
    >
      <div class="head-hos">
        <img src="../assets/logo_whyx.jpg"/>
        <div class="controller-number">QR-1600-038B</div>
      </div>
      <div class="head-title">体温单</div>
      <div class="head-info">
        <div class="item">
          科别：
          <span class="value">{{ adtLog || patInfo.dept_name }}</span>
        </div>
        <div class="item">
          姓名：
          <span class="value">{{ patInfo.name }}</span>
        </div>
        <div class="item">
          性别：
          <span class="value">{{ patInfo.sex }}</span>
        </div>
        <div class="item">
          年龄：
          <span class="value">
        ...
        <div class="item">
          出生日期：
          <span class="value date-value">{{ patInfo.date_of_birth }}</span>
        </div>
        <div class="item">
          病案号：
          <span class="value">{{ patInfo.patient_id }}</span>
        </div>
        <div class="item">
          床号：
          <span class="value">{{ bedExchangeLog || patInfo.bed_label }}</span>
        </div>
        <div class="item">
          入院日期：
          <span class="value date-value">{{ patInfo.admission_date.slice(0, 10) }}</span>
        </div>
      </div>
      <div class="table-area">
        ...
        <div class="table-box" style="transform: translateY(0.5px);">
          ...
          >
            ...
            <div class="right-label" :style="{ width: `${rightWidth}px` }"></div>
          </div>
          ...
          >
            <div
              class="label last-label"
              :style="{ width: `${leftWidth}px` }"
            ></div>
            <div class="value-item-box font-12">
              <div
                class="value-item"
                :style="middleTdStyle(index, timeLabels.length)"
                v-for="(item, index) in timeLabels"
                :key="index"
              >
                {{ item }}
              </div>
          ...
          >
            <div
              class="label last-label"
              :style="{ width: `${leftWidth}px`, alignItems: 'flex-start' }"
            >
              <div style="transform: translateY(-50%)">时间</div>
            </div>
            <div class="value-item-box last-item-box font-12">
              <div
                class="value-item"
        ...
        <div class="info-box">
          ...
          <div class="right-label-wrapper" :style="{ height: `${areaHeight}px`, width: `${rightWidth}px` }">
            <div class="right-label-content">
              <div class="right-label--title">华<br/>氏</div>
              <div 
                class="right-label--item" 
                v-for="(label, index) in fahrenheitLabels" 
                :key="label.borderColor + index" 
                :style="label"
                :data-attr="label.text"
              > 
              </div>
            </div>
          </div>
        </div>
        <div class="table-box table-box2" style="transform: translateY(-0.5px);">
          <div
            class="vtline"
            :style="{
              left: `${leftWidth + item * (6 * xSpace + 7) - 1}px`,
              'border-color': item === 7 ? 'black' : 'red',
            }"
            v-for="item in 7"
            :key="item"
          ></div>
          <div class="row font-14" :style="{ height: `${trHeight}px` }">
            ...
            <div class="value-item-box first-item-box font-14">
              <div
          ...
          <div class="row font-14" :style="{ height: `${trHeight}px` }">
            ...
            <div class="value-item-box font-14">
              <div
                class="value-item font-14"
                :style="{
                  ...smallTdStyle(index, formatPainScoreLit.length),
                  ...item.style,
                }"
                v-for="(item, index) in formatPainScoreLit"
                :key="index"
                @click="() => clickDateChangeTime(item)"
              >
                {{ item.value }}
          ...
          <div class="row font-14" :style="{ height: `${trHeight}px` }">
            <div class="label" :style="{ width: `${leftWidth}px` }">BMI(Kg/m²)</div>
            <div class="value-item-box">
              <div
                class="value-item font-14"
          ...
          <div class="row font-14" :style="{ height: `${trHeight}px` }">
            <div class="label" :style="{ width: `${leftWidth}px` }">腹围cm</div>
            <div class="value-item-box">
              <div
                class="value-item font-14"
          ...
          <div class="row font-14" :style="{ height: `${trHeight}px` }">
            <div class="label" :style="{ width: `${leftWidth}px` }">入量</div>
            <div class="value-item-box">
              <div
                class="value-item font-14"
          ...
          <div class="row font-14" :style="{ height: `${trHeight}px` }">
            <div class="label" :style="{ width: `${leftWidth}px` }">出量</div>
            <div class="value-item-box">
              <div
                class="value-item font-14"
          ...
          <div class="row last-row font-14" :style="{ height: `${trHeight}px` }">
            <div class="label last-label" :style="{ width: `${leftWidth}px` }">
              {{ customList2.label || '' }}
            </div>
            <div class="value-item-box last-item-box">
              <div
                class="value-item font-14"
      ...
      <div class="form-page-remark">
        备注：脉搏“<span class="color-red">●</span>”&nbsp;心率“<span class="color-red">○</span>”&nbsp;腋温“<span class="color-blue">×</span>”&nbsp;肛温“<span class="color-blue">○</span>”
      </div>
...
import { mockData } from 'src/projects/whyx/mockData.js'
import moment from 'moment'
import { mockDataError } from 'src/projects/whyx/mockDataError.js'
...
export default {
  ...
  computed: {
    fahrenheitLabels() {
      const startValue = 108;
      const startTop = this.ySpace * 3 + 2;
      let top = startTop;
      const labels = new Array(65).fill(0).map((_, index) => {
        const isFullValue = index % 5 === 0;
        const width = isFullValue ? 34 : 17;
        const borderColor = index === 50 ? 'red' : 'black';
        const borderWidth = isFullValue ? 1 : 0.5;
        const style = {
          text: isFullValue ? startValue - Math.floor((index / 5)) : '',
          width: width + 'px',
      ...
      for (let i = timeNumRange[0]; i < timeNumRange[1] - 1; i += timeAdd(i)) {
        ...
        for (let j = breatheList.length - 1; j >= 0; j--) {
          ...
          if (timeNum >= i && timeNum < i + timeAdd(i)) {
            if (
              ['呼吸机', 'R', 'r', '辅助呼吸'].includes(breatheList[j].value)
            ) {
              breatheList[j].value = '®'
            }
    ...
    formatPainScoreLit() {
      const timeNumRange = this.timeRange.map((x) => this.getTimeNum(x))
      const list = []
      const painScoreList = [...this.painScoreList]
      // 根据医院要求，0-5点落在当天第一个格子，21-24点落在当天最后一个格子，所以特殊处理每天第一个格子和最后一个格子的落点
      const timeNumList = this.dateList.map((x) => {
        return {
          start: this.getTimeNum(`${x} 00:00:00`),
      ...
      for (let i = timeNumRange[0]; i < timeNumRange[1] - 1; i += timeAdd(i)) {
        const item = { timeNum: i, value: '' }
        for (let j = painScoreList.length - 1; j >= 0; j--) {
          const timeNum = this.getTimeNum(painScoreList[j].time)
          if (timeNum >= i && timeNum < i + timeAdd(i)) {
            item.value = painScoreList[j].value
            item.time = `${painScoreList[j].time}`
            painScoreList.splice(j, 1)
            break
          }
        }
        list.push(item)
    ...
    operateDateList() {
      return this.vitalSigns
        .filter(
          (x) =>
            x.vital_code === '5' &&
            (x.value.includes('手术') || x.value.includes('手术入院|')),
        )
        .map((x) => x.time_point)
    },
  ...
  methods: {
      ...
      for (let i = 0; i < vitalSigns.length; i++) {
        ...
        if (this.lineMap[vitalSigns[i].vital_code]) {
          if (
            ...
            ['041', '01', '043', '042'].includes(vitalSigns[i].vital_code) &&
    ...
    xLine() {
      ...
      let preSpace = 0
      for (let i = 0; i < totalLine; i++) {
        const isBreak = i % 6 === 0 && i > 0 && i < totalLine
        const isLastLine = i === totalLine - 1;
        ...
        const params = {
          ...
          y2: this.areaHeight,
          lineWidth,
          color: isBreak && !isLastLine ? 'red' : '#000',
        }
    ...
    createText({
      ...
      if (time) {
        text.on('click', () => {
          window.parent.postMessage({ type: 'clickDateTime', value: time }, '*')
        })
      }
      this.zr.add(text)
      if (tips) {
    ...
    createBrokenLine({
      ...
      data.forEach((x, index) => {
        ...
        if (['01', '043', '041', '042'].includes(vitalCode)) {
          // 画降温
...
@media print {
  @page {
    size: a4; //定义为a4纸
    margin: 8mm 1.8cm 8mm 1.8cm; // 页面的边距
  }
  .main-view {
