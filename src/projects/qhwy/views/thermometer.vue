    ...
    <div class="table-area">
      ...
      <div class="info-box">
          ...
          <div class="item temp">
            ...
            <div class="pain-area2" :style="`height: ${painAreaHeight}px`">
              <div
                ...
                :key="item"
                :style="{
                  borderBottom: `${index == 3 ? 2 : 1}px solid #000`,
                  height: `${index == 3 ? 18 : index == 4 ? 17.7 : 16.7}px`,
                }"
              >
                <span>{{ item }}</span>
              </div>
        ...
        <div class="info-box-right" :style="{ width: `${leftWidth - 30}px` }">
          ...
          <div
            class="index-box_02"
            :style="{ height: `${ySpace * 2}px`, width: `${leftWidth - 30}px` }"
          ></div>
          <div
          ...
          <div class="main-indexBox">
            <div
              class="index-box_left item"
              :style="{ height: `${ySpace * 35}px` }"
            >
              <div class="value-item-box">
                <div
                  class="value-item"
                  :style="FahrenheitTdStyle(index)"
                  v-for="(item, index) in Fahrenheit"
                  :key="index"
                ></div>
              </div>
            </div>
            <div
              class="index-box_right item"
              :style="{ height: `${ySpace * 35}px` }"
            >
              <div class="index" v-for="item in FahrenheitList" :key="item">
                <span>{{ item }}</span>
              </div>
            </div>
          </div>
      ...
      <div class="table-box" style="transform: translateY(-0.5px)">
        ...
        <div
          ...
          :style="{ height: `${bottomTrHeight + 5}px` }"
        >
          <div class="label" :style="{ width: `${leftWidth}px` }">
            呼吸(次/分)
          </div>
        ...
        <div class="row font-14" :style="{ height: `${bottomTrHeight}px` }">
        ...
        <!-- <div class="row" :style="{ height: `${bottomTrHeight}px` }">
          ...
          <div class="value-item-box">
            <div class="value-item" v-for="(item, index) in getFormatList({ tList: customList4 })" :key="index">
              {{ item.value }}
            </div>
          </div>
        </div> -->
        <!-- <div class="row" :style="{ height: `${bottomTrHeight}px` }">
          ...
          <div class="value-item-box">
            <div class="value-item" v-for="(item, index) in getFormatList({ tList: customList5 })" :key="index">
              {{ item.value }}
            </div>
          </div>
        </div> -->
...
import { mockData } from "src/projects/qhwy/mockData.js";
...
export default {
  ...
  computed: {
    ...
    FahrenheitList() {
      const list = [];
      for (
        let i = this.FahrenheitListRange[1];
        i > this.FahrenheitListRange[0];
        i = i - 1
      ) {
        list.push(i);
      }
    ...
    Fahrenheit() {
      const list = [];
      for (
        let i = this.FahrenheitRange[1];
        i > this.FahrenheitRange[0];
        i = i - 1
      ) {
        list.push(i);
      }
  ...
  methods: {
    ...
    smallTdStyle(index) {
      return {
        color:
          index % 6 === 0 || (index - 5) % 6 === 0 || (index - 1) % 6 === 0
            ? "red"
            : "",
    ...
    FahrenheitTdStyle(index) {
      return {
        width: `${index % 5 === 0 ? this.xSpace * 2 - 7 : 15}px`,
        height: `${
          this.ySpace / 2 +
          ((index - 1) % 2 === 0 ? 1 : index % 5 === 0 ? 1 : 0)
        }px`,
        flex: "auto",
        "flex-grow": 0,
        "flex-shrink": 0,
        "border-width": `${index % 5 === 0 ? 2 : 1}px`,
        "border-color": "#000",
    ...
    handleData() {
      ...
      for (let i = 0; i < vitalSigns.length; i++) {
        ...
        if (this.lineMap[vitalSigns[i].vital_code]) {
          if (
            ["01", "02", "043"].includes(vitalSigns[i].vital_code) &&
    ...
    init() {
      ...
      this.$nextTick(() => {
        ...
        Object.values(this.settingMap).forEach((x) => {
          let data = [x.data];
          if (["01", "01", "043"].includes(x.vitalCode)) {
            // 体温为不升时，折线需要断开
            data = [[]];
            x.data.forEach((y) => {
              if (y.value <= 35) {
            ...
            x.data.forEach((y, index) => {
              ...
              if (index < x.data.length - 1) {
                ...
                if (
                  ...
                  if (x.vitalCode == "02") {
                    this.settingMap.heart.data.forEach((heartItem, ind) => {
                      const heartTimeNum = this.getTimeNum(
                        this.getLocationTime(heartItem.time)
                      );
                      if (
                        heartTimeNum <
                          this.getTimeNum(
                  ...
                  if (x.vitalCode == "20") {
                    this.settingMap.pulse.data.forEach((heartItem, ind) => {
                      const pluseTimeNum = this.getTimeNum(
                        this.getLocationTime(heartItem.time)
                      );
                      if (
                        pluseTimeNum <
                ...
                if (x.vitalCode == "02" && index == x.data.length - 1) {
                  const lastDate = {
                    time: `${this.dateRange[1]} 23:00:00`,
                    value: null,
                  };
                  x.vitalCode == "02" &&
                    this.monitoringInterval.push([
        ...
        */
        ...
        this.createNote(
          this.topSheetNote,
          this.topAreaHeight + this.indexTextAreaHeight + 1,
          "red"
        );
        // 生成表底注释
        this.createNote(
    ...
    getAreaHeight() {
      ...
      for (let i = 0; i < totalLine; i++) {
        const isBreak =
          (i - 9) % 5 === 0 && i >= 9 && i > 0 && i < totalLine - 1;
        const lineWidth = isBreak ? 2 : 1;
        preSpace +=
          lineWidth + (i === 2 || i === 3 ? this.ySpace / 2 - 1 : this.ySpace);
      }
      this.areaHeight = preSpace - this.ySpace + 2;
    },
    getAreaWidth() {
      const totalLine =
        this.xRange[1] -
    ...
    getYaxis(yRange, value, vitalCode) {
      return vitalCode == "092"
        ? ((10 - value) / (10 - 0)) * this.painAreaHeight +
            this.middleAreaHeight +
            this.timesTempAreaHeight +
            5 * this.ySpace
        : ["20", "02"].includes(vitalCode)
        ? ((this.pulseRange[1] + 10 - value) /
            (this.pulseRange[1] - this.pulseRange[0])) *
            (this.timesTempAreaHeight + 3 * this.ySpace + 2) -
          1
        : ((yRange[1] + 1 - value) / (yRange[1] - yRange[0])) *
            this.timesTempAreaHeight -
          3;
    },
    // 根据时间点计算横坐标
