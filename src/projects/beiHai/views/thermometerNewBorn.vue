<template>
  ...
  >
    ...
    <div class="head-title">婴儿记录表</div>
    <div class="head-info">
      ...
      <div class="item" style="width: 150px">
        住院号：<span class="value">{{ patInfo.patientId }}</span>
      </div>
      <!-- <div class="item" style="width:160px;flex:none;">
        入院日期：<span class="value">{{
          patInfo.admissionDate.slice(0, 10)
        }}</span>
      </div> -->
      <!-- <div class="item">
        ...
        <div class="row" :style="{ height: `${trHeight * 1.1}px` }">
          ...
          <div class="value-item-box">
            <div
              ...
              v-for="(item, index) in formatWeightList"
              :key="index"
            >
              {{ item.value }}
            </div>
          ...
          <div class="row font-14" :style="{ height: `${trHeight * 1.1}px` }">
            ...
            <div class="value-item-box">
              <div
                :style="middleTdStyle(index)"
                class="value-item font-14"
                v-for="(item, index) in getFormatVomitingList({
                  tList: vomiting,
                })"
                :key="index"
              >
          ...
          <div class="row font-14" :style="{ height: `${trHeight * 1.1}px` }">
            ...
            <div class="value-item-box">
              <div
                class="value-item font-14"
                :style="middleTdStyle(index)"
                v-for="(item, index) in getFormatList({ tList: skin })"
                :key="index"
              >
                {{ item.value.includes("黄染") ? "✔️" : "" }}
              </div>
            </div>
          </div>
          <div class="row font-14" :style="{ height: `${trHeight * 1.1}px` }">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      patInfo: {
        ...
        visit_id: "",
        admissionDate: this.parseTime(new Date()),
        age: "",
      },
      vitalSigns: [],
      typeMap: {
  ...
  computed: {
    ...
    formatWeightList() {
      ...
      for (
        ...
        for (let j = weightList.length - 1; j >= 0; j--) {
          ...
          if (timeNum >= i && timeNum < i + 2 * 4 * 60 * 60 * 1000) {
    ...
    formatStayDayList() {
      ...
      return this.dateList.map((x) => {
        // if (this.dayInterval(x, this.parseTime(new Date(), "{y}-{m}-{d}")) > 0)
        //   return "";
        return this.dayInterval(x, this.patInfo.admissionDate) + 1;
      });
    },
    formatDateList() {
  ...
  methods: {
    ...
    middleTdStyle(index) {
      return {
        width: `${this.xSpace * 2 + ((index - 2) % 3 === 0 ? 3 : 2)}px`,
        flex: "auto",
        "flex-grow": 0,
        "flex-shrink": 0,
        "border-right-style": "solid",
        "border-width": `${(index - 2) % 3 === 0 ? 2 : 1}px`,
        "border-color": `${(index - 2) % 3 === 0 ? "transparent" : "#000"}`,
        transform: "translateX(1px)",
      };
    },
    middleBCStyle(index) {
    ...
    handleData() {
      if (this.apiData.data.patientInfo)
        this.patInfo = this.apiData.data.patientInfo;
      const vitalSigns = this.apiData.data.vitalSigns.sort(
        (a, b) => this.getTimeNum(a.recordDate) - this.getTimeNum(b.recordDate)
      );
      ...
      const maxTimeNum = Math.max.apply(
        null,
        vitalSigns.map((x) => new Date(x.recordDate).getTime())
      );
      const admissionDateNum = new Date(
        `${this.patInfo.admissionDate.slice(0, 10)} 00:00:00`
      ).getTime();
      ...
      for (let i = 0; i < vitalSigns.length; i++) {
        if (
          this.getTimeNum(vitalSigns[i].recordDate) < timeNumRange[0] ||
          this.getTimeNum(vitalSigns[i].recordDate) > timeNumRange[1]-1
        ) {
          // 超出时间范围的抛弃
          continue;
        }
        ...
        if (this.lineMap[vitalSigns[i].vitalCode]) {
          if (
            ["041", "042", "043"].includes(vitalSigns[i].vitalCode) &&
            ...
            this.settingMap[this.lineMap[vitalSigns[i].vitalCode]].data.push({
              time: vitalSigns[i].recordDate,
              value: Number(vitalSigns[i].vitalValue),
            });
          }
          continue;
        }
        const item = {
          time: vitalSigns[i].recordDate,
          value: vitalSigns[i].vitalValue,
          sign: vitalSigns[i].vitalSigns,
        };
        switch (vitalSigns[i].vitalCode) {
          case "5":
          ...
          case "033":
            ...
            break;
          case "milkFrequency":
            this.milkFrequencyList.push(item);
            break;
          case "artificialFrequency":
            this.artificialFrequencyList.push(item);
            break;
          case "61":
    ...
    createBrokenLine({
      ...
      data.forEach((x, index) => {
        ...
        // if (["temperature", "043", "041"].includes(vitalCode)) {
        //   // 画降温
    ...
    // 计算底部数据的渲染列表
    getFormatList({ tList, timeInterval = 8 * 60 * 60 * 1000 }) {
      const timeNumRange = this.timeRange.map((x) => this.getTimeNum(x));
      const list = [];
    ...
    // 计算底部数据的渲染列表
    getFormatVomitingList({ tList, timeInterval = 8 * 60 * 60 * 1000 }) {
      const timeNumRange = this.timeRange.map((x) => this.getTimeNum(x));
      const list = [];
      const targetList = [...tList];
      for (let i = timeNumRange[0]; i < timeNumRange[1]-1; i += timeInterval) {
