<template>
  <div
    ...
    v-if="apiData"
  >
    <div class="head-hos" v-if="hospitalName">
      {{ hospitalName }}
    </div>
    <div class="head-title">{{ temperatureListName }}</div>
    <div class="head-info">
    ...
    <div class="table-area">
      ...
      <div class="table-box" style="transform: translateY(-0.5px);">
        ...
        <div
          :class="[
            'row',
            { 'border-top-red-2': rowIndex === 0 },
            { 'border-bottom-black-2': rowIndex === tableBottomList.length - 1 }
          ]"
          :style="{
            height:
              row.vitalCode === '04' ? `${trHeight * 2}px` : `${trHeight}px`
          }"
          v-for="(row, rowIndex) in tableBottomList"
          :key="row.vitalCode"
        >
          <div class="label" :style="{ width: `${leftWidth}px` }">
            {{
              ['32', '33', '34', '35', '36', '37'].includes(row.vitalCode)
                ? row.list.label || ''
                : row.label
            }}
          </div>
            ...
            <div
              ...
              :style="
                row.vitalCode === '062'
                  ...
                  : row.vitalCode === '04'
                  ? {
                      ...smallTdStyle(index, row.list.length),
                      ...item.style,
                      color: '#000'
                    }
                  : ''
              "
              v-for="(item, index) in ['04', '062'].includes(row.vitalCode)
                ? row.list
                : getFormatList({ tList: row.list })"
              :key="index"
            >
              {{ item.value }}
            </div>
...
import { mockData } from 'src/projects/common/mockData.js'
...
export default {
  data() {
    ...
    return {
      ...
      temperatureListName: window.temperatureListName,
      apiData: '', // 接口数据
      zr: '',
      areaWidth: 0, // 网格区域的宽度
  ...
  computed: {
    timeTds() {
      const list = window.timePointList
      const tds = []
      for (let i = 0; i < 7; i++) {
        tds.push(...list)
      }
    ...
    tableBottomList() {
      const listMap = {
        '04': this.formatBreatheList,
        '061': this.shitList,
        '062': this.formatPressureList,
        '094': this.heightList,
        '033': this.weightList,
        '091': this.inputList,
        '19': this.outputList,
      ...
      return window.tableBottomList.map((x) => {
        return {
          ...x,
          list: listMap[x.vitalCode]
        }
      })
    },
    vitalCodeMap() {
      // 将vitalCode映射key和value反转
      const codeMap = {}
      Object.entries(window.vitalCodeMap).forEach((x) => {
        codeMap[x[1]] = x[0]
      })
      return codeMap
    }
  ...
  mounted() {
    ...
    if (this.useMockData) {
      // 根据code映射转换code
      mockData.vitalSigns.forEach((x) => {
        x.vital_code = this.vitalCodeMap[x.vital_code]
      })
