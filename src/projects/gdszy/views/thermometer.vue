<template>
  ...
  >
    ...
    <div class="head-info">
      ...
      <div class="item" style="flex: none;">
        入院日期：
        <span class="value">{{ patInfo.admission_date.slice(0, 10) }}</span>
      </div>
      ...
      <div class="table-box" style="transform: translateY(-0.5px);">
        ...
        <div class="row font-14" :style="{ height: `${trHeight}px` }">
          ...
          <div class="value-item-box font-14">
            <div
              class="value-item font-14"
              v-for="(item, index) in getFormatList({ tList: pressureList1 })"
              :key="index"
            >
              {{ item.value }}
            </div>
        ...
        <div class="row font-14" :style="{ height: `${trHeight}px` }">
          ...
          <div class="value-item-box font-14">
            <div
              class="value-item font-14"
              v-for="(item, index) in getFormatList({ tList: pressureList2 })"
              :key="index"
            >
              {{ item.value }}
            </div>
        ...
        <div class="row font-14" :style="{ height: `${trHeight}px` }">
          ...
          <div class="value-item-box font-14">
            <div
              class="value-item font-14"
              v-for="(item, index) in getFormatList({ tList: pressureList3 })"
              :key="index"
            >
              {{ item.value }}
            </div>
...
import { mockData, jsonMockData } from 'src/projects/gdszy/mockData.js'
...
export default {
  ...
  computed: {
    ...
    formatPressureList1() {
      const timeNumRange = this.timeRange.map((x) => this.getTimeNum(x))
      const list = []
      const pressureList = [...this.pressureList1]
      for (
        let i = timeNumRange[0];
        i < timeNumRange[1] - 1;
        i += 3 * 4 * 60 * 60 * 1000
    ...
    formatPressureList2() {
      const timeNumRange = this.timeRange.map((x) => this.getTimeNum(x))
      const list = []
      const pressureList = [...this.pressureList2]
      for (
        let i = timeNumRange[0];
        i < timeNumRange[1] - 1;
        i += 3 * 4 * 60 * 60 * 1000
    ...
    formatPressureList3() {
      const timeNumRange = this.timeRange.map((x) => this.getTimeNum(x))
      const list = []
      const pressureList = [...this.pressureList3]
      for (
        let i = timeNumRange[0];
        i < timeNumRange[1] - 1;
        i += 3 * 4 * 60 * 60 * 1000
    ...
    formatBreatheList() {
      ...
      const xaxis = this.topSheetNote.map((x) =>
        this.getXaxis(this.getLocationTime(x.time)),
      )
      const xaxisNew = this.handleNoteXaxis(xaxis).map((item, i) => {
        return { xaxis: item, time: this.topSheetNote[i].time }
      })
    ...
    operateDateList() {
      ...
      */
      const list = this.vitalSigns.filter(
        (x) =>
          x.vital_code === '3' &&
          (x.value.includes('术') || x.value.includes('分娩')),
      )
  ...
  methods: {
        ...
        if (index < data.length - 1) {
          if (this.getNotTemTime().length) {
            for (let item of this.getNotTemTime()) {
              if (
                this.getTimeNum(data[index + 1].time) >=
      ...
      for (let i = 0; i < vitalSigns.length; i++) {
        ...
        switch (vitalSigns[i].vital_code) {
          ...
          case '1005':
            ...
            break
          case '1032':
            this.pressureList1.push(item)
            break
          case '1036':
            this.pressureList2.push(item)
            break
          case '1038':
            this.pressureList3.push(item)
            break
          case '1014':
    ...
    init() {
      ...
      this.$nextTick(() => {
        ...
        Object.values(this.settingMap).forEach((x) => {
          let data = [x.data]
          if (['2', '1001', '19'].includes(x.vitalCode)) {
            data = [[]]
          ...
          if (['1002', '1003'].includes(x.vitalCode)) {
            // 心率或脉搏过快时，折线需要断开
            data = [[]]
            x.data.forEach((y, index) => {
    ...
    createBrokenLine({
      ...
      data.forEach((x) => {
        ...
        //   this.getXaxis(this.getLocationTime(x.time))
        // );
        // const topSheetNoteTimeList = this.topSheetNote.map((x) =>x.time);
        // const xaxisNew = this.handleNoteXaxis(xaxis);
        // let sameIndex = topSheetNoteTimeList.findIndex((t)=>t==x.time)
        /**根据时间调整显示位置  根据表顶注释（表顶注释会偏移 ）偏移显示*/
        ...
        switch (dotType) {
          ...
          case 'Circle':
            ...
            if (vitalCode === '1002' || vitalCode === '1003') {
              const tList = [
        ...
        if (['1001', '2', '19'].includes(vitalCode)) {
          // 画物理降温
    ...
    getYaxis(yRange, value, vitalCode) {
      return vitalCode === 'ttpf'
        ...
        : ((yRange[1] - value) /
            (yRange[1] - (['1002', '1003'].includes(vitalCode) ? 20 : 34))) *
            this.timesTempAreaHeight -
            1
    },
    // 增加换行符
    ...
    toChinesNum(num) {
      ...
      let getWan = (temp) => {
        ...
        // 如果是两位数，一十改成十
        if (strArr.length === 2 && strArr[1] === '1001') {
...
@media print {
  @page {
    size: a4; //定义为a4纸
    margin: 5mm 10mm 5mm 13mm; // 页面的边距
  }
