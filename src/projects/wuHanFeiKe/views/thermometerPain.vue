  ...
  >
    ...
    <div class="head-info-1">
      <div class="item" style="text-align: left; flex: 1.2;">
        住院病历号：
        <span class="value">{{ patInfo.inp_no }}</span>
      </div>
    </div>
        ...
        >
          ...
          <div class="item temp color-red">
            <div class="text">
              ...
              <div class="text" style="height: 46px;"></div>
            </div>
            ...
            <div
              class="pain-area"
              :style="`height: ${painAreaHeight}px`"
    ...
    <div class="footer">
      <span>
        ...
        , 肛温
        <template><a style="color: red;">○</a></template>
        , 耳温
        <template><a style="color: red;">△</a></template>
        , 额温
        <template><a style="color: red;">★</a></template>
...
export default {
  ...
  computed: {
    ...
    formatPressureList2() {
      ...
      for (
        ...
        for (let j = pressureList.length - 1; j >= 0; j--) {
          const timeNum = this.getTimeNum(pressureList[j].time)
          if (
            timeNum > i + 10 * 60 * 60 * 1000 &&
            timeNum <= i + 14 * 60 * 60 * 1000
          ) {
            item.value = pressureList[j].value
            pressureList.splice(j, 1)
            break
    ...
    pulseList() {
      const list = []
      for (let i = this.pulseRange[1]; i > this.pulseRange[0]; i = i - 10) {
        list.push(i)
      }
      return list
    },
    ...
    createNote(notes, y, color) {
      ...
      notes.forEach((x, i) => {
        ...
        this.createText({
          ...
          x: xaxisNew[i],
          y: bottomContextList.includes(value) ? y + 5 * this.ySpace : y + 2,
    ...
    getYaxis(yRange, value, vitalCode) {
      return vitalCode == 'ttpf'
        ...
        ? ((this.pulseRange[1] + 10 - value) /
            (this.pulseRange[1] + 10 - this.pulseRange[0])) *
            this.timesTempAreaHeight +
          3 +
          2.5 * this.ySpace
