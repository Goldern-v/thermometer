  ...
  >
    ...
    <div class="table-area">
      ...
      <div class="table-box" style="transform: translateY(-0.5px);">
        ...
        <div class="row" :style="{ height: `${trHeight}px` }">
          <div class="label" :style="{ width: `${leftWidth}px` }">药物过敏</div>
          <div class="value-item-box">
            <div
              class="value-item"
...
import { mockData } from 'src/projects/quZhou/mockData.js'
...
export default {
  data() {
    ...
    return {
      ...
      vitalSigns: [],
      typeMap: {
        '16': '表顶注释', // 入院|,手术,分娩|,出院|,转入|,死亡|,排胎|,出生|,手术分娩|,手术入院|,转出|
  ...
  computed: {
    ...
    //       console.log(timeNum)
    //       if (timeNum >= i&& timeNum <=i/2 + 3 * 2 * 60 * 60 * 1000&&timeNum <i/2 + 3 * 4 * 60 * 60 * 1000) {
    ...
    handleData() {
      ...
      for (let i = 0; i < vitalSigns.length; i++) {
        ...
        if (this.lineMap[vitalSigns[i].vital_code]) {
          if (
            ['02', '03'].includes(vitalSigns[i].vital_code) &&
        ...
        switch (vitalSigns[i].vital_code) {
          ...
          case '09':
            ...
            break
          case '14':
            this.irritabilityList.push(item)
            break
          case '15':
